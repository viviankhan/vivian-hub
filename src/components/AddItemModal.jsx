...
export default function AddItemModal({ existing = null, existingRecurring = null, occurrenceDate = null, onSaveOccurrence = null, onSaveFuture = null, onDeleteOccurrence = null, onDeleteFuture = null, presetDate = null, presetText = '', presetTime = '', presetDur = null, presetCat = '', presetCats = null, presetDescription = '', presetSubtasks = null, presetReminders = null, lockDate = false, defaultRepeat = false, categories = [], routines = [], templates = [], labelModel = null, onSave, onSaveRecurring = null, onDelete = null, onDuplicate = null, onMoveToThoughts = null, onClose, title = 'Add to calendar' }) {
  ...
  // which labels you reach for: press and hold a chip and drag it up or down
  // its list. The new order is saved against the labels themselves (App
  // registers the save — see lib/labelOrder.js), so it holds in Settings and on
  // every other sheet. A plain tap is untouched: the drag only arms after a
  // short hold. A chip stays inside the section it's shown in — a folder's
  // labels rearrange among themselves, and so do the ones under "Other labels".
  const labelSection = (id) => labelMetaFor(id).folders.length ? 'records' : 'plain'
  const labelDrag = useDragReorder({
    ids: cats.map(c => c.id),
    onReorder: reorderLabels,
    groupOf: labelSection,
  ...
  const labelFolderGroups = recordFolders()
    .map(f => ({ folder: f, labels: chainCats.filter(c => labelMetaFor(c.id).folders.some(l => l.folderId === f.id)) }))
    .filter(g => g.labels.length > 0)
  const groupedLabelIds = new Set(labelFolderGroups.flatMap(g => g.labels.map(c => c.id)))
  const plainCats = chainCats.filter(c => !groupedLabelIds.has(c.id))
  ...
  const rowNodes = {
    ...
    labels: (
      <DetailRow icon={<TagIcon />} iconColor={effectiveCats.length ? headerColor : '#B7BEC8'}
        ...
        {!labelDrag.disabled && (
          <div style={{ fontSize:10.5, color:'var(--muted)', marginTop:7 }}>Press and hold a label to drag it up or down its list — the ones you use most can sit first, here and everywhere else.</div>
        )}
      </DetailRow>
    ),
    // Date
